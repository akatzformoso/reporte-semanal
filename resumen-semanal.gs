 ...
 *   1. Completar el email de Ximena en CONFIG (abajo).
 *   2. Ejecutar la función "probarResumen" → autorizar permisos → llegan los emails de prueba.
 *   3. Ejecutar la función "crearDisparador" → queda programado todos los lunes 8:00.
...
const CONFIG = {
  ...
  // Se incluyen los reportes cuya "semana que cierra" cae dentro de esta ventana
  // (9 días cubre viernes a lunes con margen para rezagados).
  diasVentana: 9,
  nombreHoja: null, // null = primera hoja de la planilla
};
...
function enviarResumenSemanal() {
  ...
  const hoja = CONFIG.nombreHoja ? ss.getSheetByName(CONFIG.nombreHoja) : ss.getSheets()[0];
  ...
  if (col('servicio') === -1) {
    MailApp.sendEmail(CONFIG.responsables[0].email, '⚠️ Resumen semanal: error de configuración',
      'No se encontró la columna "servicio" en la planilla. Encabezados: ' + datos[0].join(' | '));
    return;
  }
  const val = (fila, nombre) => {
    const c = col(nombre);
    return c === -1 ? '' : String(fila[c] || '').trim();
  };
...
function armarHtml(resp, filas, val) {
  ...
  h += '<ul>' + Object.keys(reportaron).map(s =>
    '<li><b>' + s + '</b> — ' + val(reportaron[s], 'supervisor') + ' (semana ' + val(reportaron[s], 'fecha') + ')</li>'
  ).join('') + '</ul>';
  ...
  // Secciones de contenido (solo se muestran si hay algo)
  ...
  h += bloqueCondicional('⚠️ Faltante de materia prima (OC vs factura)', filas, val, 'hayMP', 'mpObs');
  h += bloque('🍽 Cambios de menú', filas, val, 'menuDetalle');
  ...
  if (conPendientes.length) {
    ...
    conPendientes.forEach(f => {
      ...
      h += '<p><b>' + val(f, 'servicio') + ':</b> ' + faltan.join(' · ') + '</p>';
    });
  }
  ...
  h += bloque('🏥 Visita PAE Media', filas, val, 'paeNovedades');
...
function bloqueCondicional(titulo, filas, val, campoSiNo, campoDetalle) {
  ...
  let h = seccion(titulo);
  con.forEach(f => {
    h += '<p><b>' + val(f, 'servicio') + ':</b> ' + escaparHtml(val(f, campoDetalle) || '(sin detalle)') + '</p>';
  });
  return h;
}
